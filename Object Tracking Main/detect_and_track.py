...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, save_with_object_id=False, path=None,offset=(0, 0), txt_coordi_path=None, coordinates=False):
    # added a parameter txt_coordi_path that points to the path where the coordinates are to be saved
...
# ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        ...
        if coordinates:
            txt_coordi = "%i %i %i %i %i" %(id, X1, X2, Y1, Y2)
            txt_coordi += "\n"

            with open(txt_coordi_path + '.txt', 'a') as file:
                file.write(txt_coordi)
...
def detect(save_img=False):
    ...
    # ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    # save1_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
    (save_dir / 'coordinates' if save_txt or save_with_object_id else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    # ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # ||||||||||||||||||||||||||||||||||||||||||||||||||
            # save_path = str(save_dir / p.name)  # img.jpg
            txt_coordi_path = str(save_dir / 'coordinates' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt
            # ||||||||||||||||||||||||||||||||||||||||||||||||||
            ...
            if len(det):
                ...
                if len(tracked_dets)>0:
                    ...
                    draw_boxes(im0, bbox_xyxy, identities, categories, names, save_with_object_id, txt_path, (0,0), txt_coordi_path, coordinates=True if save_txt else False)
    ...
    # if save_txt:
    #     cords (save_dir, draw_boxes.bbox, p, frame, identities)
